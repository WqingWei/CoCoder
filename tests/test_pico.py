...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".cocoder" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".cocoder").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".cocoder" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "cocoder/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "cocoder.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("cocoder.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "cocoder.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("cocoder.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("cocoder.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "COCODER_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "COCODER_DEEPSEEK_API_KEY=sk-project-deepseek",
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".cocoder" / "runs"
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .cocoder/.\n"
    ...
    index_path = tmp_path / ".cocoder" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".cocoder" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".cocoder" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .cocoder/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".cocoder" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".cocoder" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/cocoder/__init__.py")
