...
def test_load_benchmark_validates_fixed_schema():
    ...
    assert Counter(task["category"] for task in benchmark["tasks"]) == {
        ...
        "recovery": 3,
        "durable-contract": 2,
        "git-diff-standalone": 4,
        "git-diff-integration": 3,
    }
...
def test_run_harness_regression_v2_writes_named_artifact(tmp_path):
    ...
    assert artifact_path.exists()
    assert artifact["summary"]["total_tasks"] == 19
...
def test_git_diff_standalone_tasks_all_pass(tmp_path):
    """独立评估实验：4 个 git_diff 独立验证任务全部通过"""
    from cocoder.evaluator import run_fixed_benchmark
    from pathlib import Path
    ...
    for row in artifact["rows"]:
        if row["id"] in standalone_ids:
            assert row["passed"] is True, f"{row['id']} failed: {row.get('failure_category')}"
            assert row["status"] == "pass"
            assert row["stop_reason"] == "final_answer_returned"


def test_git_diff_integration_tasks_all_pass(tmp_path):
    """联动实验：3 个 git_diff 联动验证任务全部通过"""
    from cocoder.evaluator import run_fixed_benchmark
    from pathlib import Path
