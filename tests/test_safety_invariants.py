...
from cocoder import FakeModelClient, MiniAgent, SessionStore, WorkspaceContext
from cocoder import cli as mini_cli
from cocoder.task_state import TaskState
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("COCODER_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("cocoder.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = mini_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["COCODER_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("cocoder.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("cocoder.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "cocoder.runtime"

    with patch("cocoder.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
