...
SCRIPTED_MODEL_OUTPUTS = {
    ...
    # ── git_diff 独立验证 ──
    "diff_changed_file": [
        '<tool name="write_file" path="config.py"><content>TIMEOUT = 60\nRETRIES = 5\n</content></tool>',
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        "<final>Done.</final>",
    ],
    "diff_unchanged_file": [
        '<tool>{"name":"git_diff","args":{"path":"utils.py"}}</tool>',
        "<final>Done.</final>",
    ],
    "diff_nonexistent_path": [
        '<tool>{"name":"git_diff","args":{"path":"不存在的文件.py"}}</tool>',
        "<final>Done.</final>",
    ],
    "diff_repeated_rejected": [
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        "<final>Done.</final>",
    ...
    # ── git_diff 联动验证 ──
    "write_then_diff": [
        '<tool name="write_file" path="config.py"><content>TIMEOUT = 120\nRETRIES = 10\nMODE = "strict"\n</content></tool>',
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        "<final>Done.</final>",
    ],
    "patch_diff_repatch": [
        '<tool name="patch_file" path="utils.py"><old_text>return True</old_text><new_text>return False</new_text></tool>',
        '<tool>{"name":"git_diff","args":{"path":"utils.py"}}</tool>',
        '<tool name="patch_file" path="utils.py"><old_text>def helper():</old_text><new_text>def helper(debug=False):</new_text></tool>',
        "<final>Done.</final>",
    ],
    "shell_then_diff": [
        '<tool>{"name":"run_shell","args":{"command":"echo VERSION=\\\"2.0\\\" >> config.py","timeout":10}}</tool>',
        '<tool>{"name":"git_diff","args":{"path":"config.py"}}</tool>',
        "<final>Done.</final>",
    ],
}
...
def _apply_task_setup(agent, task, fixture_copy_root):
    ...
    if kind == "git_init":
        subprocess.run(["git", "init"], cwd=fixture_copy_root, capture_output=True)
        subprocess.run(["git", "config", "user.email", "benchmark@test"], cwd=fixture_copy_root)
        subprocess.run(["git", "config", "user.name", "benchmark"], cwd=fixture_copy_root)
        subprocess.run(["git", "add", "-A"], cwd=fixture_copy_root, capture_output=True)
        subprocess.run(["git", "commit", "-m", "initial"], cwd=fixture_copy_root, capture_output=True)
        agent.workspace = WorkspaceContext.build(fixture_copy_root, repo_root_override=fixture_copy_root)
        return
...
class BenchmarkEvaluator:
    def __init__(
        ...
        self.workspace_root = Path(workspace_root) if workspace_root is not None else Path(
            tempfile.mkdtemp(prefix="cocoder-benchmark-")
        )
    ...
    def run_task(self, task):
        ...
        session_store = SessionStore(fixture_copy_root / ".cocoder" / "sessions")
        run_store = RunStore(fixture_copy_root / ".cocoder" / "runs")
