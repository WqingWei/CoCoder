...
DEFAULT_CORE_REPORT_PATH = Path("docs/metrics/cocoder-benchmark-core-report.md")
...
def build_stress_agent_metrics():
    with tempfile.TemporaryDirectory(prefix="cocoder-metrics-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        workspace = WorkspaceContext.build(workspace_root)
        store = SessionStore(workspace_root / ".cocoder" / "sessions")
        agent = Pico(
...
def _run_memory_variant(mode):
    with tempfile.TemporaryDirectory(prefix="cocoder-memory-experiment-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _run_memory_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="cocoder-memory-large-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="cocoder-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".cocoder" / "sessions")
...
def run_security_experiment_suite(repetitions=3):
    ...
    for scenario_id, runner in SECURITY_SCENARIOS:
        for _ in range(repetitions):
            with tempfile.TemporaryDirectory(prefix="cocoder-security-exp-") as temp_dir:
                workspace_root = Path(temp_dir)
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _provider_profile(provider):
    ...
    if provider == "gpt":
        api_key = provider_env("COCODER_OPENAI_API_KEY", ("OPENAI_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "COCODER_OPENAI_API_KEY or OPENAI_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("COCODER_OPENAI_MODEL", ("OPENAI_MODEL",), "gpt-5.4"),
            "base_url": provider_env("COCODER_OPENAI_API_BASE", ("OPENAI_API_BASE",), "https://api.openai.com/v1"),
            "api_key": api_key,
        }
    if provider == "deepseek":
        api_key = provider_env("COCODER_DEEPSEEK_API_KEY", ("DEEPSEEK_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "COCODER_DEEPSEEK_API_KEY or DEEPSEEK_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("COCODER_DEEPSEEK_MODEL", ("DEEPSEEK_MODEL",), "deepseek-v4-pro"),
            "base_url": provider_env("COCODER_DEEPSEEK_API_BASE", ("DEEPSEEK_API_BASE",), "https://api.deepseek.com/anthropic"),
            "api_key": api_key,
        }
    api_key = provider_env(
        "COCODER_ANTHROPIC_API_KEY",
        ("ANTHROPIC_API_KEY", "COCODER_RIGHT_CODES_API_KEY", "RIGHT_CODES_API_KEY", "COCODER_OPENAI_API_KEY", "OPENAI_API_KEY"),
    )
    if not api_key:
        return {"provider": "claude", "status": "blocked", "reason": "COCODER_ANTHROPIC_API_KEY or ANTHROPIC_API_KEY missing"}
    return {
        "provider": "claude",
        "status": "ready",
        "model": provider_env("COCODER_ANTHROPIC_MODEL", ("ANTHROPIC_MODEL",), "claude-sonnet-4-6"),
        "base_url": provider_env("COCODER_ANTHROPIC_API_BASE", ("ANTHROPIC_API_BASE",), "https://www.right.codes/claude/v1"),
        "api_key": api_key,
    }
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="cocoder-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="cocoder-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _run_real_repeated_call_scenario(provider):
    with tempfile.TemporaryDirectory(prefix="cocoder-real-security-repeat-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with tempfile.TemporaryDirectory(prefix="cocoder-real-security-") as temp_dir:
                workspace_root = Path(temp_dir)
...
def _run_recovery_task_variant(task, variant):
    with tempfile.TemporaryDirectory(prefix="cocoder-recovery-ablation-") as temp_dir:
        workspace_root = Path(temp_dir)
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
