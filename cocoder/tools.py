...
from .workspace import IGNORED_PATH_NAMES, MAX_TOOL_OUTPUT, clip
...
BASE_TOOL_SPECS = {
    ...
    "git_diff": {
        "schema": {"path": "str='.'", "staged": "bool=False", "context": "int=3"},
        "risky": False,
        "description": "Show git diff for a file or directory.",
    },
}
...
TOOL_EXAMPLES = {
    ...
    "git_diff": '<tool>{"name":"git_diff","args":{"path":"src/auth.py"}}</tool>',
}
...
def tool_git_diff(agent, args):
    path = agent.path(args.get("path", "."))
    staged = str(args.get("staged", "")).strip().lower() in {"true", "1", "yes"}
    context = int(args.get("context", 3))
    ...
    if not output:
        return f"(no changes in {relative})"

    count = output.count("\n@@")
    header = f"# git diff {relative}"
    if staged:
        header += " --staged"
    if count:
        header += f" ({count} hunk{'s' if count != 1 else ''})"
    return header + "\n" + clip(output, MAX_TOOL_OUTPUT)
